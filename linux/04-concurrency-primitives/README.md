...
glibc 的 `pthread_mutex_t` 和 Go `sync.Mutex` 都会在阻塞路径上使用 futex-style 机制（Go 不走 glibc/pthreads 封装，而是自己实现 futex-like 原语、直接通过 `syscall` 调 Linux `futex(2)`）。HotSpot Java 锁应先按 JVM monitor / AQS / parking 路径理解：无争用或轻量路径可能完全停留在 JVM/user-mode；发生竞争并进入阻塞/parking 时，才可能最终落到 pthread/futex 这类 OS-backed 同步机制。
...
→ **回链**：`linux-handson/03-process-model`（`/proc/<pid>/status` 看 `voluntary_ctxt_switches` 验证锁争用的上下文切换次数）；`cli-toolbox/01-process-and-job-control.md`（用 `wchan=futex_wait` 识别 futex-backed 等待，例如锁/条件变量阻塞）
...
`signal()` 的典型用法是：生产者通常在持有 mutex 时改变共享状态，然后通知一个 condition waiter。被通知者只是离开 condition wait queue，后面仍要重新抢 mutex，所以 signal 不等于「马上运行」。
...
POSIX 只定义条件变量的语义：`wait` 释放 mutex 并等待条件通知；返回用户代码前必须重新拿到 mutex。POSIX 不规定底层必须使用 futex，也不把 requeue 作为语义合同。

在 Linux 上，condition variable 的实现可以借助 futex 计数器和 requeue 操作来降低 `broadcast` 成本。例如 `FUTEX_CMP_REQUEUE` 可以在比较 futex 值仍符合预期时，把一部分 waiter 从 condition futex 的等待队列迁移到 mutex futex 的等待队列，而不是把所有 waiter 都唤醒后一起抢同一把 mutex。语义仍然是「条件已通知，接下来排队抢 mutex」；requeue 只是实现优化，不是 POSIX 层的承诺。`FUTEX_WAIT_REQUEUE_PI` 属于优先级继承 mutex 的特殊 PI 路径，本章只需要知道它更特殊、这里不展开。
...
**惊群（thundering herd）**：从高层语义看，`notifyAll`（Java）/ `Broadcast`（Go `sync.Cond`）会通知所有等待线程，但共享谓词通常只能被少数线程消费。若实现直接唤醒大量 waiter，或等待者很多且缺少 requeue、限流等缓解，调度和抢锁开销会被放大；最终仍只有拿到 mutex 且重新检查谓词成功的线程能继续。解决方式：能确定只需要一个等待者继续时用 `Signal`，或改用信道（Go channel 的 select 语义天然避免此问题）。
