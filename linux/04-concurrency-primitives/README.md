...
- **x86-64**：`LOCK CMPXCHG`——`LOCK` 前缀锁住内存总线（或利用 cache 一致性协议），保证在操作期间其他 CPU 无法读写该内存地址。
...
glibc 的 `pthread_mutex_t`、Java HotSpot 的 thin lock / biased lock、Go `sync.Mutex` 全部基于此机制（Go 用的是自己实现的 futex-like 原语，不直接调 Linux futex）。
...
**解读**：200,000 次加锁操作中，只有 **3 次** 触发了 `futex` 系统调用。其余 199,997 次完全在用户态以 CAS 完成，不进内核。这 3 次是两个线程真正发生争用（一方需要等待另一方）时的内核介入——正是 futex「无争用不进内核」设计的实证。
...
条件变量的语义：
1. `wait`：原子地「释放持有的互斥锁 + 把自己睡在等待队列上」（两步必须原子，否则会错过唤醒）。内核实现为 `futex(FUTEX_WAIT_REQUEUE_PI, ...)` 或类似操作。
...
→ **回链**：`linux-handson/04-memory-model`（`/proc/<pid>/status` VmRSS 等内存指标，以及数据竞争排查）；`linux/01-memory-primitives/`（虚拟内存模型 + 为什么需要内存屏障的场景原点）
