...
| 名词 | 中文抓手 | 在哪里 | 本章里怎么用 |
|---|---|---|---|
| `PC` / `rip` | 指令指针，CPU 下一步要执行哪条机器指令 | CPU 寄存器状态；`rip` 是 x86-64 名字，PC 是通用叫法 | 上下文切换必须保存它，否则恢复后不知道从哪条指令继续跑 |
| `SP` / `rsp` | 栈指针，当前线程栈顶地址 | CPU 寄存器状态；`rsp` 是 x86-64 名字，SP 是通用叫法 | 它不是整条栈，只是指向栈内存的地址；保存它就能重新接回调用链 |
| `ALU` | 算术逻辑单元，真正做加减、比较、位运算的执行部件 | CPU 执行单元内部 | `a + b` 这类计算通常是先把值放进寄存器，再由 ALU 算 |
| `syscall` | x86-64 上进入内核的特殊指令，也可泛指系统调用机制 | CPU 指令 + Linux 内核入口约定 | 用户态不能直接操作内核资源，要通过它切到内核态 |
| `svc #0` | aarch64 上进入内核的特殊指令，Supervisor Call | CPU 指令 + Linux 内核入口约定 | 和 x86-64 的 `syscall` 是同类动作：从用户态陷入内核态 |
| `ABI` | 二进制调用约定，规定参数/返回值/寄存器保存规则 | 编译器、库、内核、CPU 架构之间的约定 | 解释为什么 `write(fd, buf, len)` 编译后参数会出现在固定寄存器里 |
| trap frame | 陷入内核时保存的一份用户态 CPU 现场 | 通常在当前任务的内核栈上 | 里面保存用户态 PC、SP、通用寄存器等，内核返回用户态时按它恢复 |
| `CR3` / `TTBR` | 页表基址寄存器，告诉 CPU 当前地址空间用哪套页表 | CPU 特权寄存器；`CR3` 常见于 x86，`TTBR` 常见于 ARM | 进程间切换可能要改它；线程共享同一地址空间时通常不用改 |
| `TLB` | 地址翻译缓存，缓存“虚拟地址 → 物理地址”的翻译结果 | CPU 内部缓存 | 换页表后很多翻译失效，重新填充 TLB 是上下文切换成本来源之一 |

再补几个会在执行过程中出现的内存层次词：

| 名词 | 中文抓手 | 怎么理解 |
|---|---|---|
| cache line | cache 一次搬运/缓存的最小块，常见 64 字节 | CPU 不是只缓存一个 int，而是把相邻一小段内存一起带进 cache |
| DRAM | 主内存，平时说的“内存条” | 容量大但远；L1/L2/L3 都没命中时才去这里取 |
| store buffer | 写缓冲 | CPU 写内存时通常先把写入放到缓冲/缓存里，之后再逐层落到更下层 |
| FPU/SIMD | 浮点/向量计算寄存器和执行单元 | 处理浮点、向量、加密压缩等批量计算；上下文切换时可能也要保存 |
| link register | 链接寄存器，常见于 aarch64 的 `x30` | 保存函数返回地址；如果函数还要继续调用别人，通常会把它保存到栈上 |
...
> CPU 执行的是机器指令，不是源码行。`PC/rip` 是指令指针，指向下一条机器指令；`SP/rsp` 是栈指针，指向当前线程栈顶。指令做计算时主要操作寄存器，真正的加法、比较、位运算由 ALU（算术逻辑单元）完成。寄存器是 CPU 内部少量最快的工作区；栈是线程虚拟地址空间里的一段内存；L1/L2 cache 是硬件自动维护的内存副本。函数调用时，ABI 规定参数和返回值优先走寄存器，放不下或需要保存现场时才写到栈上，所以寄存器和栈里会出现类似内容，但本质不同。syscall 时，用户态按约定把 syscall 编号和参数放进寄存器，x86-64 执行 `syscall`，aarch64 执行 `svc #0` 进内核。内核会保存用户态的 PC、SP 和寄存器现场。上下文切换时也是保存这些 CPU 现场和恢复线索，不复制整条栈；恢复后线程从原来的机器指令继续跑。

### 3 分钟版

更完整的回答可以按时间线展开：

1. **先讲普通指令执行**：源码会被编译成机器指令，CPU 用 `PC/rip`（指令指针）找到下一条指令。像 `c = a + b` 这种语句，运行时通常不是直接在栈上相加，而是把值装进寄存器，然后 ALU（算术逻辑单元）在寄存器里完成加法。
...
3. **再讲栈和函数调用**：栈不是 CPU 里的结构，而是每个线程的一段内存；`SP/rsp` 是栈指针寄存器，它指向当前栈顶。函数调用会形成栈帧，里面可能有返回地址或 link register（链接寄存器）的保存值、保存的 frame pointer、保存的寄存器、局部变量、spill 区和栈上传参区。优化后很多变量不会进栈，可能一直在寄存器里。

4. **再讲为什么寄存器和栈内容相似**：ABI 规定函数参数和返回值优先放寄存器，参数太多或结构太大才放栈。寄存器不够时，编译器会 spill 到栈。调用函数前后，为了保护 caller-saved / callee-saved registers，也会把寄存器值暂存到栈。所以内容可能相似，但角色不同：寄存器是执行工作区，栈是调用链和后备存储。
