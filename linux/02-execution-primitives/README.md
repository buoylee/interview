...
源码
  ...
  → PC/rip 指向下一条指令
  → 指令把值装进寄存器
  → ALU 在寄存器上做加法/比较/地址计算
  → 需要内存数据时，通过地址访问 L1/L2/L3/内存
  → 函数调用时用栈帧保存返回地址、局部变量、溢出的寄存器
  → syscall 时按 ABI/内核约定把参数放寄存器，执行 syscall/svc
  → CPU 进内核，内核保存用户态寄存器现场到内核栈
  → 如果要调度，内核保存当前任务恢复线索，切到下一个任务
  → 恢复时还原 PC/SP/通用寄存器，线程从原来的机器指令继续跑
```

这条链里有几个最容易混的概念：寄存器、栈、cache。先把三者底层身份分清：
...
> CPU 执行的是机器指令，不是源码行。`PC/rip` 指向下一条指令，指令做计算时主要操作寄存器。寄存器是 CPU 内部少量最快的工作区；栈是当前线程虚拟地址空间里的一段内存；L1/L2 cache 是硬件自动维护的内存副本。函数调用时，ABI 规定参数和返回值优先走寄存器，放不下或需要保存现场时才写到栈上，所以寄存器和栈里会出现类似内容，但本质不同。syscall 时，用户态按约定把 syscall 编号和参数放进寄存器，执行 `syscall` / `svc` 进内核。内核会保存用户态的 PC、SP 和寄存器现场。上下文切换时也是保存这些 CPU 现场和恢复线索，不复制整条栈；恢复后线程从原来的机器指令继续跑。

### 3 分钟版

更完整的回答可以按时间线展开：

1. **先讲普通指令执行**：源码会被编译成机器指令，CPU 用 `PC/rip` 找到下一条指令。像 `c = a + b` 这种语句，运行时通常不是直接在栈上相加，而是把值装进寄存器，然后 ALU 在寄存器里完成加法。
...
3. **再讲栈和函数调用**：栈不是 CPU 里的结构，而是每个线程的一段内存；`SP/rsp` 才是寄存器，它指向当前栈顶。函数调用会形成栈帧，里面可能有返回地址或 link register 的保存值、保存的 frame pointer、保存的寄存器、局部变量、spill 区和栈上传参区。优化后很多变量不会进栈，可能一直在寄存器里。
...
| syscall 和普通函数调用最大区别是什么？ | 普通函数调用仍在用户态；syscall 触发特权级切换，进入内核入口，并保存/恢复用户态现场。 |
