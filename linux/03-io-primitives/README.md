...
- 内核缓冲 / page cache 本章会引用，深度讲解见 [`linux/01-memory-primitives/`](../01-memory-primitives/README.md)。
...
> ② 讲透，无干净 5 行复现（需要 `/proc/meminfo` 对比 + 磁盘延迟测量，环境依赖多），跳过 ③。page cache 底层已在 `linux/01-memory-primitives/` 的 VSZ/RSS 原语里讲过，本节聚焦 IO 视角。
...
**page cache 是内核管理的文件数据缓存**（见 `linux/01-memory-primitives/` 的 mmap/page cache 原语）：
...
| 触发 | 说明 |
|------|------|
| 后台定时 | `dirty_writeback_centisecs`（默认 500 cs = 5s），pdflush 周期扫描脏页写盘 |
...
→ **回链**：`linux-handson/05-io-and-files` §page cache 与 fsync 实验（`/proc/meminfo` 的 `Cached` 字段、`vmstat -s` 看 page-in/out）；`linux/01-memory-primitives/` §mmap（文件 mmap 就是直接映射到 page cache 里同一批物理页）
...
| **用法场景** | 简单、安全；Nginx、Netty 默认用 LT | 高性能、低通知次数；需要正确处理 EAGAIN |
| **Go net poller** | 用 ET，配合 goroutine 调度器 |
