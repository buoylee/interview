...
`limit_req`(漏桶 + `burst` + `nodelay` 三者行為差異)、`limit_conn`(並發連線數)、`limit_rate`(頻寬限速)、`limit_req_status`/`limit_conn_status`、共享記憶體 zone 大小怎麼估;**邊界**:Nginx 本地限流計數是每節點的(同節點 worker 共享記憶體、跨節點不共享),跨節點全域配額要外移。
