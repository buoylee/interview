...
- Mermaid 必须记录精确 block hash，并通过图类型、稳定节点 ID、术语和关键 node-edge 的静态结构检查。
- 图示必须进入独立语义 review，确认图中因果关系、正文解释和术语一致。
- 若可信本地 CLI 或隔离浏览器可用，推荐实际渲染作为补充视觉 QA；环境不可用时，只要 hash、静态检查和语义 review 通过，就不得阻塞验收或提交。
...
- 核心图使用可编辑 Mermaid，并为每个 block 记录精确 hash。
- Mermaid 的图类型、稳定节点 ID、术语和关键 node-edge 通过静态检查。
- 独立 review 确认图示因果关系、正文和术语一致；实际渲染是推荐但非阻塞的补充视觉 QA。
