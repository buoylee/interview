...
17 章交付后按五条标准复盘,发现四块成体系缺口,增补:**17 文件 I/O 与序列化、18 安全基础、19 实战骨架(过渡实战)、20 编码面试工具箱(面试编码轮)**,并把 `weakref`/`cached_property`/`total_ordering`/`operator`/`contextvars`/`enum` 深化等 P1 散点折入现有章。边界不变(并发/性能/web 仍指针外链)。详见 plan 的"增补"节。
