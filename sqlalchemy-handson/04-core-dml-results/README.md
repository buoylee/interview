...
本章延續 production question、prediction、failing/naive behavior、mechanism、corrected
behavior、evidence、decision 與 interview drill 八個階段。案例使用 SQLAlchemy 2.0
公開 API 與真實 PostgreSQL；production function 接受 caller 提供的 `Connection`，可以
`execute()`，但不會自行 `begin()` 或 `commit()`。application-service entry point 才擁有交易
邊界。
...
4. product 價格更新為 12.50，因此 tenant report 的總庫存價值是 `8 × 12.50 = 100.00`。
...
- [`test_core_dml_scenario_records_expected_result_shapes`](../lab/tests/integration/test_catalog.py)
  鎖定 scenario 的六行 observation contract。

## SELECT、JOIN、subquery、CTE、window function

**Mechanism**：`inventory_report()` 先以 `select()` 將 inventory 與 product JOIN。JOIN 不只比對
...
**Failing/naive behavior** 是把 Python list 逐列迴圈，每列各呼叫一次 `execute()`；或先 SELECT
目前庫存、在 Python 加 quantity，再 UPDATE。前者把一個 set-oriented 意圖拆成多次 round trip；
後者在兩個 writer 同時讀到舊值時可能 lost update。另一個錯誤是 lower-level function 自行
commit，令 application service 無法把 tenant、product、inventory 與後續工作放在同一個原子
交易。

`create_tenant()` 示範單筆 `INSERT`。scenario 則把同一個 `tenants.insert()` 與兩個同 shape 的
mapping 一次傳給 `Connection.execute()`；parameter sequence 表達 executemany，而不是兩次
application-level execute。`preserve_rowcount=True` 只為本 lab 將 INSERT rowcount 在 cursor
關閉前保存，以留下可重現的 `2`；rowcount 在不同 DBAPI、RETURNING 與 executemany 組合中不
一定有相同語義，不能拿它當跨資料庫 business proof。DML constructors、execution-time
parameter set 與 `RETURNING` 的契約見
...
**Mechanism**：`upsert_product()` 使用 PostgreSQL dialect 的 `insert()`，並指定 named
...
**Corrected behavior**：每個 statement 先明確選擇回傳欄位，再立刻選擇 Result consumption
shape。catalog 的單列 DML 使用：
...
**Evidence**：[`ch04-core-dml-results.md`](../lab/evidence/ch04-core-dml-results.md) 是 scenario
...
第一行證明本次環境在保存 rowcount 後看見兩筆 tenant insert；它不證明 network packet 數、
server plan 或所有 DBAPI 都會回傳 `2`。後五行共同證明 conflict identity、更新後 RETURNING
shape、atomic increment、version 與 window aggregate 的關聯。`tenant_stock_value=100.00` 也
間接證明 report 讀到 upsert 後的 12.50，而不是原始 10.00。
