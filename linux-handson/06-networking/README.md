...
### 看单连接质量与重传:`ss -ti` + `nstat`

前面的 `ss` 都在看连接的**状态/数量**;要看**单条连接好不好**(延迟、有没有重传),给 `ss` 加 `-i`(info,挖出 TCP 内部状态):

```console
$ ss -ti dst :6379
ESTAB 0 0 127.0.0.1:54012 127.0.0.1:6379
      cubic rtt:23.4/11.2 retrans:0/5 ...        # 只截关键字段
#                  ↑rtt 往返时延/抖动(ms)   ↑retrans 重传(进行中/累计)
```
- `rtt:23.4/11.2` = 往返时延 / 抖动(ms),越大链路越慢越抖;
- `retrans:0/5` = 重传(当前 / 累计):**一个段没等到 ACK(被丢了),内核就再送一遍**——这条连接累计重送过 5 个段;
- ⚠️ `ss -ti` 只显示**当下还活着的连接**。连上就关的短命连接,手动敲多半抓到空 → 压测时用 `while :; do ss -ti dst :6379; done` 狂刷,或开一条长连接(`redis-cli` 不关)再看。

要看**全机重传总账**(不挑连接),用 `nstat` 读内核 TCP 计数器,配 `-n` 取差值:

```console
$ nstat -n                              # 设基准、本次不输出
$ # …跑压测 / 制造流量…
$ nstat | grep -i retrans
TcpRetransSegs                 1234     0.0    # 这次新增的重传段数
```

> 这里只教「怎么看」;重传 / 拥塞控制的**原理**(超时重传 vs 快重传、RTT 怎么算)见本章末「延伸指针」→ `04a-network-tcp-core.md`,不重复啃。
