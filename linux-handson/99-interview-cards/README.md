...
| 问题 | 一句话答 |
|------|----------|
| 什么是进程? | 程序的一次运行实例,也是 OS 分配/隔离资源的基本单位;进程管地址空间/fd/权限,线程才是执行流 |
...
| `cmd \| while read` 变量为何丢? | 管道右侧在子 shell;用 `while ... done < <(cmd)` 进程替换(接 01) |
| grep/sed/awk 各擅长? | grep 找行、sed 改行、awk 按列处理+统计 |
| 统计 top N IP? | `awk '{print $1}' \| sort \| uniq -c \| sort -rn \| head` |
| `trap` 干嘛? | 注册信号/退出处理(接 03),做清理或脚本级优雅关闭 |
| 怎么防 `rm -rf` 删错? | `set -u` + `"${DIR:?}"`,变量永远加引号 |
...
**30 秒口头答**
> 进程是程序的一次运行实例,也是操作系统分配和隔离资源的基本单位。它拥有独立的虚拟地址空间、文件描述符表、权限身份、信号处理方式等资源;进程里至少有一个线程,真正被 CPU 调度执行的是线程。简单说:进程解决资源隔离和管理,线程解决并发执行。

**展开**
- 程序是磁盘上的静态文件;进程是程序跑起来后的实例。同一个程序可以启动多个进程。
- 进程像一个资源容器:地址空间、堆/栈/mmap、fd 表、环境变量、当前工作目录、uid/gid、信号处理方式都挂在它上面。
- 进程也是隔离边界:进程 A 不能直接读写进程 B 的内存;要读文件、发网络包、创建子进程,都必须通过 syscall 请内核代劳。

**追问预案**
- *Q:进程和线程区别?* → 进程偏资源边界,线程偏执行流;同一进程内线程共享地址空间和 fd 表,但各有自己的栈、寄存器和调度状态。
- *Q:Linux 内核里进程和线程是两种对象吗?* → 不是,底层都叫 `task_struct`;区别主要取决于 `clone()` 时共享了哪些资源。
- *Q:程序和进程区别?* → 程序是静态代码/文件,进程是代码被加载后的一次运行实例,有 PID 和运行时资源。

**踩坑/数据点**
> 不要把进程只理解成「会被 CPU 调度的东西」。面试里更稳的说法是:进程是资源容器和隔离边界;线程才是执行流/调度对象。
...
### 卡 10-A:写运维/CI 脚本,开头为什么要 `set -euo pipefail`?

**30 秒口头答**
> 因为 bash 的默认行为对自动化很危险:命令失败了脚本会继续往下跑、用了拼错的变量当空字符串处理、管道中间环节失败还被判成功。`set -e` 让任何命令失败立即终止;`-u` 让引用未定义变量直接报错——能挡住 `rm -rf "$DIR/"` 里 `$DIR` 没设导致删根目录这类灾难;`-o pipefail` 让 `curl ... | grep ...` 里 curl 失败也算整条失败。配合变量永远加引号、删除路径用 `"${DIR:?}"`,脚本才健壮。

**追问预案**
- *Q:`set -e` 有什么例外/坑?* → 在 `if`/`&&`/`||` 条件里的命令失败不会触发退出(这是设计);需要某条允许失败时用 `cmd || true`。
- *Q:为什么 CI 里特别重要?* → 没有 `-e` 时中间步骤失败但脚本返回 0,CI 显示成功、问题流到生产,属于「静默失败」。
- *Q:`-o pipefail` 解决什么具体场景?* → 管道默认只看最后一条命令的退出码,前面的失败被吞;开了它整条管道才如实反映失败。

**踩坑/数据点**
> 真实事故:部署脚本 `cd /app/$VERSION && rm -rf *`,`$VERSION` 因上游为空 → `cd /app/` 成功后在 /app 删光所有版本。`set -u` + 引号 + `"${VERSION:?}"` 三道保险任一道都能拦住。
