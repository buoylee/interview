...
模型先用当前参数做预测，loss 衡量预测和目标差多少；反向传播计算每个参数对应的 gradient；optimizer 再根据这些 gradient 更新参数，让下一次预测更接近目标。
...
梯度消失和梯度爆炸来自同一个直觉：深层网络里，gradient 要穿过很多层。传播时会反复乘上每一层的局部变化率。
...
没有这些结构，深层 Transformer 更难稳定训练；有了它们，模型才能把很多层堆起来，同时保持 optimizer update 有效。
