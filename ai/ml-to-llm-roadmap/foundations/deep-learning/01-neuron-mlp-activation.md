...
它让输出不必总是围绕原点变化，相当于给线性变换一个可学习的平移量。没有偏置，模型能表达的函数会更受限制。
...
它的关键作用是引入非线性。没有激活函数，多层线性层叠在一起仍然会坍缩成一个线性变换：
...
```text
x -> Linear -> Activation -> Linear -> Activation -> ...
```
...
下一步可以读 [FFN 与门控](./04-ffn-gating-for-transformer.md)，看现代 Transformer 怎样在基础 MLP 上加入门控结构。
