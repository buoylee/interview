# 2.4 CNN & Residual Connection（Day 8-9）
...
> **面试优先级：⭐（CNN 本身）/ ⭐⭐⭐（Residual Connection）**
>
> **LLM 工程师需要了解 CNN 吗？**
> - CNN 本身：**不需要深入**。面试不会让你讲卷积公式或 VGG 架构
> - Residual Connection：**必须掌握**。Transformer 每一层都在用，面试必考
> - ViT：了解即可，帮助理解多模态模型（GPT-4V、LLaVA）
>
> **速读建议**：如果时间紧，**直接跳到第 4 部分（Residual Connection）**，其余浏览即可。
>
> **先修**：[01-神经网络基础](./01-neural-network-basics.md)（知道梯度消失是什么）
...
1. 理解卷积的直觉（局部连接 + 权值共享）— 用 1 分钟看懂即可
2. **重点掌握 Residual Connection** → 这是对后续最重要的知识，面试必考
3. CNN 的具体架构（VGG、GoogLeNet）了解即可，面试不会深问

## ⏭️ 下一节预告

下一节是整个阶段 2 最重要的一节：**RNN → LSTM → Attention 的演进**。这条线直接连接到 Transformer。你会理解为什么 Attention 被发明、为什么 Transformer 取代了 LSTM — 这些是面试高频题。
