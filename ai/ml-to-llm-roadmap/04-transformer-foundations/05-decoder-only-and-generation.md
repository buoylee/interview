...
- Transformer Block 的组成；不熟先看 [05-transformer-block.md](./05-transformer-block.md)。
- Self-Attention 会让 token 读取上下文；不熟先看 [04-self-attention-qkv.md](./04-self-attention-qkv.md)。
...
如果只记名字，面试时很容易答虚。可以这样理解：

- **Encoder-only**：只保留 Transformer Encoder。输入 token 之间通常可以双向互看，适合把一段文本编码成 token 表示或句向量。BERT 类模型常用于分类、NER、embedding、rerank 等“理解型”任务，但它不是天然逐 token 生成模型。
- **Encoder-Decoder**：Encoder 先双向理解输入，Decoder 再用 causal mask 逐步生成输出，并通过 cross-attention 读取 Encoder 的结果。T5、早期翻译和摘要模型常用这种范式，适合“给定输入，生成另一个序列”的 seq2seq 任务。
- **Decoder-only**：只保留 Transformer Decoder 的自回归生成部分，把 prompt、历史对话、检索文档、工具 schema 都拼成同一个 token 序列，然后不断预测下一个 token。GPT、LLaMA 这类通用 LLM 主要采用这个范式。

本篇后面重点讲 Decoder-only，不是因为另外两种不重要，而是因为 AI Engineer 面试里最常追问的流式输出、上下文窗口、KV Cache、Function Calling、推理成本，都主要发生在 Decoder-only 自回归生成链路上。Encoder-only 在 RAG 的 embedding/rerank 里仍然常见；Encoder-Decoder 在翻译、摘要和结构化转换任务里仍然有价值。
...
- BERT、T5、GPT 架构有什么区别？
- Encoder-Decoder 里的 cross-attention 负责什么？
...
5. Encoder-only、Encoder-Decoder、Decoder-only 分别更适合什么任务？
