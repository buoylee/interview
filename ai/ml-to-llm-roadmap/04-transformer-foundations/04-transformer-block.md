...
- Residual、LayerNorm、初始化的作用；不熟先看 [normalization/residual foundation（后续任务创建）](../foundations/deep-learning/03-normalization-residual-initialization.md)。
- FFN 和门控激活；不熟先看 [FFN/gating foundation（后续任务创建）](../foundations/deep-learning/04-ffn-gating-for-transformer.md)。
