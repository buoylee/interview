...
> 📖 **怎么实现**：这里讲的是「metadata 能按 date/version 排除过期内容」这个概念。但「系统凭什么知道哪份最新、文档局部更新怎么办、WHERE 该加在哪一层召回通道」是另一回事——实现见 Lab 簇 [Hybrid、Rerank 与调试 · 版本过滤怎么落地](../../rag-lab/03-hybrid-rerank-debugging.md)。
...
「dense / sparse」这两个名字描述的是**向量的形状**，不是「用没用 AI」。把文本变成向量有两种做法：

```text
sparse  一维对应词表里一个词,词表几万维,一篇文档只用到其中几十个词
        [0, 0, 0, 3, 0, ..., 1, 0, 0]   ← 几万维,99.9% 是 0 → "稀疏",本质是精确词面匹配
dense   embedding 把语义压进一个短向量,每一格都是非 0 浮点
        [0.12, -0.88, 0.31, ...]        ← 几百~一千多维,格格有值 → "稠密",比的是语义方向
```

一句话：稀疏 = 长而空的词表向量（词面），稠密 = 短而满的语义向量（意思）。这也直接解释了它们的强弱——稀疏靠精确 token 命中（编号、缩写、人名强，同义改写弱），稠密靠语义方向（同义改写强，罕见精确词可能漏）。两者互补，正是后面要做 hybrid 的原因。
...
> 📖 **深入**：黑盒里一个 chunk 到底怎么存(行结构)、不建索引时的暴力扫描、HNSW/IVF-PQ 内部存了什么、加 metadata 过滤为什么会「性能塌陷」、高并发怎么扩(pgvector vs Milvus)——这一层是 RAG 资深面试高频区，单独展开见 [向量库内部：一个 chunk 怎么存、怎么被搜出来](./02b-vector-store-internals.md)。
...
| 稀疏 = 没用神经网络、稠密 = 用了 | dense/sparse 分的是向量形状（长而稀 vs 短而密）；SPLADE 等 learned sparse 用了模型，却仍产出词表维度的稀疏向量 |
| recall 高就代表 RAG 好 | 后面的排序、组装和生成仍会影响答案 |
