...
| 防死循环 | `max_turns` 手写(`loop.py` 第 50 行 `for turn in range`) | `step_budget` 计数(`supervisor.py`) + `recursion_limit` 护栏 |
...
| 流式 | 要自己改 `create(stream=True)` 再透传 | `astream_events` 现成(API 层 `/chat/stream` SSE) |
...
| 多路由专家 | 无(只有 kb 问答) | supervisor LLM 路由:kb_rag / web_agent / human_review / FINISH |
...
| 指标 | 实测值 | 备注 |
|------|--------|------|
| MCP `call_mcp_tool` 冷启动(3 次) | 0.202 s / 0.209 s / 0.225 s | 每次调用都 `spawn` 新子进程 + `initialize` 握手;生产应长连接复用 session |
| `search_docs` 进程内(3 次) | 0.0002 s / 0.0001 s / 0.0001 s | 纯 Python 关键词打分,无 I/O;< 0.5 ms |

裸 loop 的代价:每次 `read_doc` 多付 ~200–225 ms 的子进程冷启动。如果一轮对话触发 2 次 `read_doc`,仅 MCP 启动就消耗 ~0.4–0.45 s,与 LLM 调用延迟同量级。
...
- 要 token 级流式且多节点(`astream_events` 现成)
...
| 简化点 | 详情 |
|--------|------|
| MCP 冷启动 | 每次调用 `call_mcp_tool` 都 spawn 新子进程 + `initialize` 握手,实测 202–225 ms;生产应长连接复用 session(`async with ClientSession` 持久化) |
...
| 无 budget 控制 | 只有 `max_turns` 上限;MVP 有 `step_budget` 字段 + `recursion_limit` 双层护栏 |
