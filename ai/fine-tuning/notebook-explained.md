...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    max_seq_length = 2048,
    # ⭐ 最大序列长度 = 模型一次能"看到"的 token 数量上限
    # 整个对话（输入 + 输出）的 token 总数不能超过这个值
    # 超出的部分会被截断（模型看不到）
    #
    # 2048 ≈ 约 1500-2000 个中文字，大多数数学题+解答够用
    #
    # 为什么不设更大？因为 Attention 的显存消耗 ∝ seq_len²：
    #   seq_len=2048 → Attention 矩阵 2048×2048 ≈ 4M
    #   seq_len=4096 → 4096×4096 ≈ 16M（4 倍显存！）
    #   seq_len=8192 → 8192×8192 ≈ 67M（16 倍显存！）
    #
    # 类比：这是"答题纸"的大小
    #   2048 = A4 纸，够写大多数题
    #   8192 = A3 纸，能写长推导，但更贵（更多显存）
