...
- `src/hooks/useCancelRequest.ts:87` / `handleCancel`：运行中优先 abort foreground signal；空闲时 pop queued command。
...
- `src/utils/messageQueueManager.ts:322` / `clearCommandQueue()`：ESC / kill agents 等取消路径清空 queue。
