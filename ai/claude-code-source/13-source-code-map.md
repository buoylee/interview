...
- `src/tools/AgentTool/AgentTool.tsx:278` / `AgentTool.call()`：teammate、normal subagent、fork subagent 的分流入口；校验 team restriction、agent type permission、required MCP servers、isolation、recursive fork。
- `src/tools/AgentTool/AgentTool.tsx:414` / fork prompt branch：fork path 复用 parent rendered system prompt、调用 `buildForkedMessages()`；normal path 构造 selected agent system prompt 和 simple user prompt。
...
- `src/tools/AgentTool/runAgent.ts:332` / initial subagent state：生成 `agentId`、注册 perfetto、构造 `initialMessages = filterIncompleteToolCalls(forkContextMessages) + promptMessages`、选择 file-state cache、解析 user/system context。
...
- `src/tasks/LocalAgentTask/LocalAgentTask.tsx:149` / `queuePendingMessage()` 与 `drainPendingMessages()`：`SendMessage` 写入 running subagent mailbox，subagent 下一次 attachment collection 时清空并读取。
...
- `src/tools/SendMessageTool/SendMessageTool.ts:149` / local message routing helpers：普通 teammate mailbox 与 local task message path 的分界。
- `src/tools/SendMessageTool/SendMessageTool.ts:520` / `SendMessageTool`：对 running local agent 调用 `queuePendingMessage()`；对 stopped/background local agent 可触发 `resumeAgentBackground()` 继续执行。
