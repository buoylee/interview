...
Claude Code 的权限系统不是让模型自己“保证安全”，而是在模型输出 `tool_use` 之后，由 runtime 根据当前 permission mode、规则、工具自己的权限检查、用户交互和 sandbox 能力再做一次授权。模型可以表达意图，例如“我要运行 Bash”或“我要 Edit 文件”，但它没有最终执行权。

核心链路是：启动时 `initializeToolPermissionContext()` 汇总 CLI、settings、policy、session 和工作目录信息；工具执行前 `canUseTool` 进入 `hasPermissionsToUseTool()`；先检查 whole-tool deny，再检查 whole-tool ask，然后调用工具自己的 `checkPermissions()` 做内容级判断。工具级 deny、强制交互、内容 ask 和 safety check 会先被尊重；只有这些没有拦住，才进入 bypass、always-allow 和 auto mode fast path。Bash 还会单独判断 shell 规则、危险命令和 sandbox。sandbox 是运行时 containment，不是 prompt 里的道德约束：它决定命令是否在受限环境里 spawn，而不是让模型“不要做坏事”。
...
第二层是 tool_use 前的决策器。模型发出工具调用后，runtime 先处理 whole-tool deny 和 whole-tool ask，再让工具自己的 `checkPermissions()` 对路径、命令、文件内容等做更细判断。工具内容级结果处理完之后，才看 bypass、always allow、auto mode classifier/allowlist 等是否允许继续。
...
- `auto`：优先用 classifier、safe-tool allowlist、acceptEdits fast path 和 denial tracking 做自动裁决；但它不是“永不询问”，在 classifier 上下文过长、classifier 不可用或 fail-open、连续拒绝达到限制、需要 review 的安全检查等情况下，仍可能回落到 ask、deny 或 review 路径。危险 shell allow 规则会在进入 auto 时被识别并清理或提示。
...
危险权限清理集中在 `permissionSetup.ts`。`findOverlyBroadBashPermissions()` 会识别 `Bash(*)` 这类过宽 shell allow；auto mode 还会识别 `Bash(python:*)`、PowerShell 下载执行等可能绕过 classifier 的规则。进入 auto 的模式切换由 `transitionPermissionMode()` 触发 `stripDangerousPermissionsForAutoMode()`，后者再调用底层 `removeDangerousPermissions()` 移除可更新来源中的危险 allow，并把剥离的规则暂存起来，离开 auto 时再恢复。
...
sandbox 决策在 `src/tools/BashTool/shouldUseSandbox.ts:130`。它先看 `SandboxManager.isSandboxingEnabled()`；如果 sandbox 全局关闭就返回 false；如果输入显式 `dangerouslyDisableSandbox` 且 policy 允许 unsandboxed commands，也返回 false；没有 command 返回 false；命中用户配置的 excluded commands 同样返回 false。其他 Bash 命令默认使用 sandbox。源码注释特别强调 excluded commands 是便利功能，不是安全边界。
...
- `src/utils/permissions/permissions.ts:1158` / `hasPermissionsToUseToolInner()`：whole-tool deny、whole-tool ask、tool `checkPermissions()`、工具级结果、bypass、allow、auto 的核心顺序。
...
3. `canUseTool` 调用 `hasPermissionsToUseTool()`，先处理 whole-tool deny / ask，再调用 `BashTool.checkPermissions()` 做命令级判断。
4. 工具级 deny、强制交互、内容 ask 或 safety check 如果没有拦截，才继续走 bypass、always-allow、用户批准 ask 或 auto classifier/allowlist fast path。
...
sandbox 不可用并不等于命令自动禁止。`shouldUseSandbox()` 在 sandbox 全局关闭、没有 command、命中 excluded commands，或 `dangerouslyDisableSandbox` 被 policy 允许时都会返回 false；后续是否执行仍由权限规则、工具检查和 policy 控制。excluded commands 只是用户便利配置，不是安全边界。真正的安全点是“权限裁决”和“执行 containment”两条线各自生效。
...
flowchart TD
  ...
  B --> C{"whole-tool deny?"}
  C -->|yes| R["reject tool_result"]
  C -->|no| D{"whole-tool ask?"}
  D -->|yes| E["approval request"]
  D -->|no| T["tool checkPermissions"]
  T -->|tool-level deny| R
  T -->|forced interaction or content ask| E
  T -->|safety check| E
  T -->|passthrough or allow| F{"mode and fast paths"}
  E -->|rejected| R
  E -->|approved| G{"Bash?"}
  F -->|dontAsk| R
  F -->|bypass or always-allow| G
  F -->|auto classifier or allowlist allows| G
  F -->|auto classifier blocks or denial-limit deny| R
  F -->|auto ask or review fallback| E
  G -->|no| X["execute tool"]
  G -->|yes| S{"sandbox decision"}
  S -->|use sandbox| XS["execute in sandbox"]
  S -->|sandbox disabled / no command / excluded commands / approved override| XU["execute without sandbox"]
  XS --> O["tool_result"]
  XU --> O
  X --> O
```
...
第三，auto mode 不是简单的 YOLO。它有 classifier、safe allowlist、acceptEdits fast path、危险 shell permission cleanup 和 denial tracking，目标是在减少用户打断的同时保留 ask、deny、review、拒绝限制等可解释的回退路径。
...
3. auto mode 和 default mode 最大差异是什么？
   default mode 遇到 ask 倾向于问用户；auto mode 会优先尝试 classifier、allowlist 和 acceptEdits fast path，并跟踪连续拒绝，但在 classifier 不可用、上下文过长或达到拒绝限制等情况下仍可能 ask、deny 或进入 review。
