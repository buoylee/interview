...
MultiEdit 在运行时表现为多处编辑的同类 mutation。源码地图中能看到 `TOOL_VERBS` 把 `MultiEdit` 归入 Editing；具体版本若没有独立 `MultiEditTool` 符号，也应按文件编辑的同一原则理解：多个 edit 需要共享路径权限、文件新鲜度和 diff/review 语义，不能退化为模型自由输出补丁。
...
- `src/bridge/sessionRunner.ts:70` / `TOOL_VERBS`：包含 Read、Write、Edit、MultiEdit、Bash 的用户可见动词映射。
...
4. MultiEdit 应该怎样理解？
   它是多处文件编辑的批量语义，仍应继承 Edit 的路径权限、新鲜度、diff/review 和失败反馈，而不是把多个替换变成无约束 patch。
