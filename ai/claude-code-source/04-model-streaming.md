...
Claude Code 的 model streaming 不是“把字一个个打到屏幕上”这么简单。它把一次模型响应拆成两类运行时事件：一类是可以立即展示的增量内容，例如 text、thinking、stream mode；另一类是必须等结构完整后才能变成协议消息的 durable assistant message，尤其是 `tool_use`。`queryModelWithStreaming()` 负责从 API 边界产出 stream event，并在每个 `content_block_stop` 处 yield 一个“完成的 content block 对应的 assistant message”；`query()` 负责消费这些事件、更新 UI、收集 `tool_use`、必要时提前启动工具，并把这些 assistant message 放回 transcript。

流式路径的核心价值是降低 coding agent 的等待时间：用户能立刻看到模型是否在思考、是否开始写解释、是否正在构造工具输入；同时 runtime 可以在 `tool_use` block 一旦完成并被 yield 后尽早调用工具，而不是等整段 assistant 输出结束。最终送回下一轮模型的仍然是标准消息序列：assistant 的 `tool_use` message 后面跟 user 的 `tool_result` message。也就是说，streaming 优化的是执行时序和交互反馈，不改变模型协议里的因果关系。

如果 streaming 失败或被降级，`queryModelWithoutStreaming()` 会通过同一层 `queryModel()` 消费完整 generator，再由 `query()` 处理返回的 assistant message。差别是用户看不到中间 token，工具也不能在 `tool_use` block 刚完成时边流边启动；但持久 transcript、tool schema、permission、tool_result 回填这些语义保持一致。

## 这一章解决什么问题

这一章解释 Claude Code 如何把模型的流式响应接入 agent loop：

- 为什么 coding agent 需要 streaming：缩短首 token、展示长工具输入、及时发现工具调用、让并发安全工具提前开跑。
- partial assistant text 和 `tool_use` block 的差异：text 可以作为 UI 增量展示，`tool_use` 必须携带稳定 `id`、`name`、完整 `input`，才能成为可执行的运行时任务。
- streaming 输出如何变成 durable assistant message：API stream 在 `content_block_start` / `content_block_delta` 中累积单个 block，在 `content_block_stop` 处 yield 对应该 block 的 `AssistantMessage`；后续 `message_delta` 再补 usage 和 stop reason。
...
第一层是 API 流。`src/services/api/claude.ts` 里的 `queryModelWithStreaming()` 包住 `queryModel()`，让调用方能收到 `StreamEvent`、每个完成 content block 对应的 `AssistantMessage` 或 synthetic API error。`queryModelWithoutStreaming()` 也调用同一个 `queryModel()`，只是消费完整个 generator 后返回捕获到的 assistant message。

第二层是 durable transcript。stream event 只是 UI 和运行时调度信号；真正会进入下一轮模型上下文的是已经完成的 assistant content block message、工具执行产生的 user `tool_result` message、以及少量 attachment/meta message。`query()` 在模型流和工具执行推进过程中，把这些 durable message 组合成下一次 state。

第三层是工具执行的提前启动。启用 `useStreamingToolExecution` 时，`query()` 一收到完成的 `tool_use` block 对应的 assistant message，就把它交给 `StreamingToolExecutor.addTool()`。执行器可以在模型响应尚未完全结束时启动工具，但结果仍以 `tool_result` message 的形式产出，并按需要保持顺序。
...
streaming 期间，`queryModel()` 逐个消费 API stream part。`message_start` 建立 partial message，`content_block_start` 根据 block 类型初始化 text、thinking、`tool_use` 等 content block，`content_block_delta` 用 `text_delta`、`thinking_delta` 或 `input_json_delta` 继续填充当前 block。到了 `content_block_stop`，`queryModel()` 会把这个完成的 content block 单独 normalize 成一个 `AssistantMessage` 并立刻 yield；随后 `message_delta` 到达时，再把真实 usage 和 stop reason 直接写回最近 yield 的 message。UI 层的 `handleMessageFromStream()` 会根据这些 stream event 切换 responding、thinking、tool-input、tool-use 等显示状态，并维护临时 streaming text / streaming tool use。

对 `query()` 来说，真正关键的是 stream 中可能多次 yield 的 `AssistantMessage`。每收到一个完成 content block 对应的 assistant message，`query()` 会：

- 把它加入 `assistantMessages`，作为 durable transcript 的一部分。
- 从 `message.content` 里筛出 `type === "tool_use"` 的 block，追加到 `toolUseBlocks`。
- 将 `needsFollowUp` 置为 true，表示本轮模型输出包含工具调用，还需要执行工具并再次进入模型。
- 如果存在 `StreamingToolExecutor` 且没有 abort，就把每个完成的 `tool_use` block 交给 `addTool()`，让工具尽早排队执行。
...
非流式 fallback 走 `queryModelWithoutStreaming()` 时，API 调用仍经过 `queryModel()` 的 schema 构建、message normalization 和 retry/fallback 逻辑。只是调用方不会逐个处理 stream event，而是在 generator 被完整消费后拿到 assistant message。对于工具系统来说，这意味着无法在 `tool_use` block 刚完成时提前启动，但后面的 `toolUseBlocks` 收集、`runTools()` 编排、`tool_result` 回填仍然相同。
...
- `src/query.ts:827`：把完成 content block 对应的 assistant message 加入 `assistantMessages`。
...
- `src/services/api/claude.ts:752`：`queryModelWithStreaming()`，对外暴露 stream event 和 assistant message。
- `src/services/api/claude.ts:1120` 起：API 请求前的 tool search、tool filtering、tool schema 构建和 message normalization。
- `src/services/api/claude.ts:1930` 起：真实 API stream 消费，处理 `message_start`、`content_block_start`、`content_block_delta` 等 event。
- `src/services/api/claude.ts:2171`：`content_block_stop`，将单个完成 content block normalize 成 assistant message 并 yield。
- `src/services/api/claude.ts:2229`：`message_delta`，把最终 usage 和 stop reason 回写到最近 yield 的 assistant message。
- `src/utils/messages.ts:2930`：`handleMessageFromStream()`，UI 层根据 stream event 更新显示状态。

## 关键数据结构与状态

- `AssistantMessage`：可持久化的 assistant 响应片段。stream 中每个完成的 content block 会形成一个 assistant message，进入 `assistantMessages`。
...
4. 当 `content_block_stop` 让一个 assistant message yield 出来，`query()` 将其持久化到 `assistantMessages`。
5. 如果这个 message 含 `tool_use`，`query()` 记录 block，并在 streaming executor 打开时立即 `addTool()`。
...
API 层还有 stall、timeout 和非流式 fallback。`claude.ts` 会记录 stream event 之间超过阈值的 stall；非流式 fallback 有独立 timeout，避免后端卡住时让 CLI 无限等待。对调用方而言，这些失败最终都要么 yield synthetic API error message，要么返回可处理的 assistant message，避免半截消息直接进入 durable transcript。
...
sequenceDiagram
    ...
    API-->>Q: content_block_start / content_block_delta(text_delta, thinking_delta, input_json_delta) / content_block_stop
    Q-->>UI: transient streaming state
    API-->>Q: AssistantMessage for completed content block
...
Claude Code 选择把 streaming event 和 durable message 分开，是为了兼顾交互体验和协议正确性。UI 可以快速响应 partial text，但 transcript 只保存完成 content block 对应的 assistant message，避免把不完整 JSON、半个 tool input 或无效 thinking signature 写入历史。
...
答：不会。它只改变“什么时候可见、什么时候能开始执行”。assistant message 仍被 `query()` 扫描 `tool_use`，工具仍经过 permission/call/result，下一轮仍看到 `tool_result`。
