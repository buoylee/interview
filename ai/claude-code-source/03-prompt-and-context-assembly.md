...
- `src/services/api/claude.ts` / `toolToAPISchema()`：把 resolved tool pool 中的工具转换成模型可见 tool schema；`hasPendingMcpServers` 主要影响 ToolSearch 是否保留。
...
4. tool schema 是不是 system prompt 的一部分？
   不是。工具通过 `tools` 参数进入模型请求，并在 API 边界经 `toolToAPISchema()` 变成模型可见工具描述和输入 schema；权限、执行、中断由 runtime 控制。
