...
SDK/print 的 `QueryEngine.submitMessage()` 也会调用 `fetchSystemPromptParts()`。它的组装更 headless：`customSystemPrompt` 存在时跳过 default `getSystemPrompt()` 和 `getSystemContext()`，最终用 custom 或 default，加上 memory mechanics prompt 和 append prompt。这保证 SDK 可以用自定义系统提示，同时复用 query loop。

### User context 与 system context

`src/utils/queryContext.ts` 的 `fetchSystemPromptParts()` 并行获取三块 cache-key 前缀：`defaultSystemPrompt`、`getUserContext()`、`getSystemContext()`。当 custom system prompt 存在时，它会跳过 default prompt 和 system context，因为 custom 已经替换默认系统层。
...
工具定义通过 `toolUseContext.options.tools` 进入 `query()`，再传给 `deps.callModel({ tools })`。模型可见的是工具名、描述、输入 schema 和 API 层需要的 tool metadata；runtime-only 的 `canUseTool`、permission mode、AbortController、setState、file cache 不会作为自然语言上下文暴露给模型。

MCP tools 也在请求 options 中体现：`query()` 会传 `mcpTools: appState.mcp.tools`、`hasPendingMcpServers`，并且 prompt 层可能包含 MCP instructions。MCP server instructions 有两种形态：普通 system prompt section，或在 delta 机制开启时通过 `mcp_instructions_delta` attachments 持久化，减少 late MCP connect 对 prompt cache 的破坏。
...
- `fetchSystemPromptParts()` 把 system prompt、user context、system context 作为 cache-key 前缀统一获取。
...
- `src/QueryEngine.ts` / `QueryEngine.submitMessage()`：SDK/print 路径组装 headless prompt/context。
- `src/query.ts` / `query()`：把 system prompt、messages、tools 和 context 传入模型请求，并把 tool_result/attachments 拼入下一轮。
...
3. REPL 用 `buildEffectiveSystemPrompt()` 应用 override/coordinator/agent/custom/default/append precedence；SDK/print 用 headless 规则组装 custom/default + memory mechanics + append。
...
- custom system prompt 会跳过 default `getSystemPrompt()` 和 `getSystemContext()`。这给 SDK/CLI 用户强控制力，但也意味着默认的部分系统上下文不会自动出现。
...
4. tool schema 是不是 system prompt 的一部分？
   不是。工具通过 `tools` 参数进入模型请求，模型可见工具描述和输入 schema；权限、执行、中断由 runtime 控制。
