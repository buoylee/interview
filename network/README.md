...
└────────────────────────────────────────────────────┘
  ...
  ║ 纵切线（横跨所有层，不属于某一层）                     ║
  ║   端到端排查.md                                        ║
  ║   （一个 URL 请求的完整链路 + 二分逐跳定位 + 症状鉴别）  ║
  ╧══════════════════════════════════════════════════════╧
```

> 上面按层切的文档各讲一层；`端到端排查.md` **纵着切一刀**，把一个请求穿过所有层的路径串成一条可排障的线。按层学完想「把它们连起来排障」时读它。
...
- [`端到端排查.md`](./端到端排查.md) — **先读这篇**：全链路图 + 二分逐跳定位 + 「症状→哪一跳」决策树（不知道从哪查时的入口）
...
| 文档 | 讲什么 | 不讲什么 |
|---|---|---|
| [端到端排查.md](./端到端排查.md) | 一个 URL 请求的完整链路图、二分逐跳定位法、「症状→哪一跳」鉴别诊断、深挖指针 | 命令用法、协议本体、抓包细节（全部链接到对应文档） |
