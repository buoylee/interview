...
> **先建立"两个时钟"的心智**:内存回收是两套机制按不同节奏在跑——**引用计数时刻在转**(每次引用增减实时更新、归零立即回收,99% 的内存是它收的);**分代 GC 偶尔才跑**(攒够一批新对象才扫一次,专补引用计数治不了的循环)。一句话:**计数器收大头、GC 兜循环。** 下面分别看。
...
- **什么时候触发**:不是定时器,而是按**「分配数 − 释放数」累计超过阈值**才扫第 0 代(`gc.get_threshold()` 默认 `(700, 10, 10)`:第 0 代净增 700 个对象触发一次扫描,每满 10 次 0 代回收才扫一次 1 代,2 代更稀)。所以 GC 是"攒够一批才来扫",既不是每次分配都扫、也不是按时间走——这跟时刻在跑的引用计数正好是两种节奏。
