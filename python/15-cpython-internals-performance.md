...
### 未来:3.13 free-threaded + JIT

- **3.12+** 做了不少解释器提速(自适应特化字节码)。
- **3.13** 引入**实验性 JIT**(把热点编译成机器码)和**可关 GIL 的 free-threaded 构建**(真多线程并行)。这俩成熟后会显著改变上面的结论,但短期内生产仍以"有 GIL、无 JIT"为基线。面试提一句是加分项。
...
| 多核计算 | 线程真并行 | goroutine 真并行 | 受 GIL,需多进程/扩展 |
...
**Q6. 听说过 3.13 的变化吗?**
3.13 引入了**实验性 JIT**(热点编译为机器码)和**可关 GIL 的 free-threaded 构建**(真多线程并行)。它们成熟后将削弱"Python 慢""多线程跑不满 CPU"的传统结论,但当前生产仍以有 GIL、解释执行为基线。
