...
> ⚠️ **前置（重要）**：本篇是**速查手册**，默认你已经有 OS 地基。如果你对 `us`/`sy`/`wa`、`RSS`/`Swap`、`await`、`TIME_WAIT` 这些字段**本身**还不熟、看着发懵，**别从这篇硬啃** —— 先去 [`linux-handson/`](../../linux-handson/)（尤其 [`03 进程`](../../linux-handson/03-process-model/)→[`07 排查方法论`](../../linux-handson/07-troubleshooting-playbook/)，那里用 `stress-ng` 实验**让你亲眼看着这些数字动起来**），把地基打通，再回来把本篇当速查卡用。
...
|------|------|---------|
| `us` | 用户态 CPU | 高 → 业务代码消耗 |
| `sy` | 内核态 CPU（执行 syscall 的时间） | 高 → 系统调用频繁：I/O、网络，**也包括内存分配/缺页、锁竞争(futex)、进程/线程创建**。用 `strace -c -p <pid>` 看到底卡在哪类 syscall |
