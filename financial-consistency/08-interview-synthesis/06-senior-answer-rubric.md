...
问题：还需要说明业务事实是否闭合、unknown 怎么处理、死信后如何修复、如何检验不变量。
...
> 我会把这个系统分成执行闭环和事实闭环。执行闭环处理请求、状态和消息；事实闭环通过账本、渠道账单、供应商账单、清结算和对账 Case 建立证据链，让每笔资金变化可追溯、可解释。高风险修复进入 maker-checker，所有修复追加事实并可复核。系统用故障注入、历史检查和上线后监控持续检验核心不变量，同时用准实时和日终对账监控差异率、修复时效和长期挂起风险。

## 评分维度

| 维度 | 低分表现 | 高分表现 |
| --- | --- | --- |
| 事实所有权 | 混淆日志、消息、报表、workflow history、consumer offset 和事实，把执行痕迹误当成金融事实 | 明确领域事实、账本事实、外部事实和派生视图 |
