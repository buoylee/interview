...
failed="$(jq -cn --argjson row "$row" --slurpfile o "$observations" --slurpfile r "$recovery" --arg run "$run_id" '
  $o[0] as $x | $r[0] as $recovery | [
    ...
    (if $x.watermark_run_id==$run and $x.target_watermarks.passed==true and $x.target_watermarks.mysql_revision >= $x.source_watermark and $x.target_watermarks.elasticsearch_revision >= $x.source_watermark then empty else "target_watermark" end),
    ...
    (if $x.verification.run_id==$run and $x.verification.status=="PASS" and $x.verification.conclusive==true and $x.verification.stable==true and $x.verification.exact_managed_field_diff_count==0 and $x.verification.version_metadata_diff_count==0 then empty else "independent_verification" end),
