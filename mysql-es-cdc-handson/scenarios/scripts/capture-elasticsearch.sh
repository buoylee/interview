...
output="$(jq -n --arg index "$index" --slurpfile docs "$tmp/hits" '{schema_version:1,consistency:"ELASTICSEARCH_PIT",index:$index,documents:$docs[0]}')"
output="$(jq -c -f "$(dirname "$0")/lib/sort-capture.jq" <<<"$output")";if test -n "$target";then printf '%s\n' "$output"|atomic_json "$target";else jq -S .<<<"$output";fi
