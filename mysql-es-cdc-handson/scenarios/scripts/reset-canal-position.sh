...
wait_meta_stable_after(){ local old_hash="$1" output="$2" current previous= changed_at=$SECONDS start=$SECONDS;while true;do copy_meta "$output";current="$(hash_file "$output")";if [[ "$current" != "$old_hash" ]];then if [[ "$current" != "$previous" ]];then previous="$current";changed_at=$SECONDS;fi;if ((SECONDS-changed_at>=2));then decode_meta "$output" >"${output%.dat}.json";return;fi;fi;if ((SECONDS-start>=120));then die "timeout waiting for stable ACK-derived meta transition";fi;sleep 0.2;done; }
...
copy_meta "$evidence_dir/reset-anchor-meta-before-publication.dat"
decode_meta "$evidence_dir/reset-anchor-meta-before-publication.dat" >"$evidence_dir/reset-anchor-meta-before-publication.json"
[[ "$(hash_file "$evidence_dir/reset-anchor-meta-before-publication.dat")|$(jq -r .journal "$evidence_dir/reset-anchor-meta-before-publication.json")|$(jq -r .position "$evidence_dir/reset-anchor-meta-before-publication.json")" == "$old_hash|$old_journal|$old_position" ]] || die "reset=true cursor advanced before anchor publication"
curl -fsS -X POST "http://127.0.0.1:8083/internal/rebuild/runs/${run_id}/canal-recovery/barriers/${anchor_run_id}" -H 'Content-Type: application/json' -d "{\"kind\":\"RESET_ANCHOR\",\"preOffsets\":$anchor_before}" >"$evidence_dir/reset-anchor-publication.json"
jq -S '.offsets' "$evidence_dir/reset-anchor-publication.json" >"$evidence_dir/reset-anchor-offsets.json"
jq -S '.events' "$evidence_dir/reset-anchor-publication.json" >"$evidence_dir/reset-anchor-events.json"
anchor_offsets="$(cat "$evidence_dir/reset-anchor-offsets.json")"
before_hash="$old_hash"
wait_meta_stable_after "$before_hash" "$evidence_dir/canal-meta-reset.dat"
[[ "$(jq -r .journal "$evidence_dir/canal-meta-reset.json")" != "$old_journal" ]] || die "ACK-derived cursor retained purged journal"
...
curl -fsS -X POST "http://127.0.0.1:8083/internal/rebuild/runs/${run_id}/canal-recovery/barriers/${sentinel_run_id}" -H 'Content-Type: application/json' -d "{\"kind\":\"NORMAL_SENTINEL\",\"preOffsets\":$restart_after}" >"$evidence_dir/normal-sentinel-publication.json"
jq -S '.offsets' "$evidence_dir/normal-sentinel-publication.json" >"$evidence_dir/normal-sentinel-offsets.json";sentinel_offsets="$(cat "$evidence_dir/normal-sentinel-offsets.json")"
jq -S '.events' "$evidence_dir/normal-sentinel-publication.json" >"$evidence_dir/normal-sentinel-events.json"
