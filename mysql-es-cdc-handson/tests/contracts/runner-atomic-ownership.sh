...
M6_EVIDENCE_ROOT="$evidence" M6_RUNNER_FIXTURE="$good" bash "$runner" duplicate-event >/dev/null 2>&1
new_target="$(readlink "$evidence/duplicate-event")";test "$new_target" != "$old_target";jq -e '.result=="PASS"' "$evidence/duplicate-event/result.json" >/dev/null
old_target="$new_target";old_hash="$(shasum -a 256 "$evidence/duplicate-event/result.json"|awk '{print $1}')"

set +e;M6_EVIDENCE_ROOT="$evidence" M6_RUNNER_FIXTURE="$fixture" M6_RUNNER_FAIL_STAGE=before-replace bash "$runner" duplicate-event >/dev/null 2>&1;replace_rc=$?;set -e
test "$replace_rc" -ne 0;test "$(readlink "$evidence/duplicate-event")" = "$old_target";test "$(shasum -a 256 "$evidence/duplicate-event/result.json"|awk '{print $1}')" = "$old_hash"
