...
for root in "$module_root/target/classes" "$module_root/target/test-classes"; do
  ...
  while IFS= read -r -d '' class_file; do
    if LC_ALL=C grep -aEiq 'com[.]interview[.]mysqlescdc[.]consumer([^[:alnum:]_]|$)|com/interview/mysqlescdc/consumer([^[:alnum:]_]|$)|search-sync-consumer|java/lang/reflect' "$class_file"; then
