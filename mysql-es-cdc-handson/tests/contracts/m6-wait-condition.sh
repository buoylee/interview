...
test "$rc" -eq 124
test ! -s "$tmp/out"
jq -e '. as $diagnostic | .status=="TIMEOUT" and .description=="timeout" and ([7,124]|index($diagnostic.last_exit_code))!=null and .attempts>=2 and .duration_ms>=900 and .duration_ms<3000 and .last_stdout=="last-stdout" and .last_stderr=="last-stderr"' "$tmp/error" >/dev/null
