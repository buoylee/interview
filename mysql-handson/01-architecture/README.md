...
一个具体数字帮助定位性能：一台普通 MySQL 8.0 实例，**默认 `max_connections=151`**（生产常调到 500-2000）。每个连接有两部分内存：一是**固定的线程栈**（`thread_stack` 默认 1 MB），二是**按需分配的会话缓冲**（`sort_buffer_size`/`join_buffer_size`/`read_buffer_size` 等，跑大查询/排序时会临时叠上去，单连接可冲到几十 MB）。所以「每连接 ~1 MB」只是**空闲基线**——2000 个空闲连接光线程栈就 2 GB 起步，而高并发跑复杂查询时实际占用还要乘几倍，这才是「连接数打满 + 大查询」一起来会 OOM 的根源。
