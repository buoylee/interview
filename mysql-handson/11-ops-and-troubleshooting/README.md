...
**(b)** 备份怎么做才能在 RTO（Recovery Time Objective，允许恢复花费的最长时间）和 RPO（Recovery Point Objective，允许丢失数据的最大时间窗口）要求内恢复；
...
- [01（架构师整合）- 写风暴下 redo × Buffer Pool × checkpoint 怎么互相拖垮](scenarios/01-write-storm-checkpoint-throttle.md) — 跨 ch02/04/06/07 的综合大题：实测 checkpoint age 被自适应刷脏摁在 83%（redo=100MB 顶住）vs 钉死在 81% + 写入被限流（redo=32MB），揭示「redo 太小不报错、只悄悄压 TPS」
