...
**一句话**: 先区分“索引不能用于定位”“只用到部分 key part”和“索引可用但成本模型没选”；三者不能都叫成整个索引失效。

**90 秒**:
- **最左前缀断链**: `WHERE b=1`（跳过 a）→ 联合索引 (a,b,c) 失效
- **索引列上做函数**: `WHERE YEAR(create_time)=2024` → 改为 `create_time BETWEEN '2024-01-01' AND '2024-12-31'`
- **隐式类型转换**: 字符串列用整数比较 `WHERE phone=13800138000`（phone 是 varchar）→ 按数值比较，原字符串索引不能定位；不代表所有异型比较都会失效
- **LIKE 前缀通配**: `LIKE '%abc'` 失效，`LIKE 'abc%'` 有效
- **OR 跨列**: 各分支有合适索引时可能走 Index Merge；某一分支无可用索引时更容易退化全表扫
- **优化器放弃**: 选择率太低（如 status 只有 0/1）时可能选全表扫；这是成本选择，不是索引结构失效
...
**一句话**: 先按高频查询需要的最左前缀、排序和分组设计；等值列通常放在范围列前，区分度只作为同等候选间的参考。

**90 秒**:
- **查询模式优先**: 先看其他 SQL 能否复用 `(a)`、`(a,b)` 等左前缀，以及是否还要支持 ORDER BY / GROUP BY
- **等值 > 范围**: `(user_id, create_time)` 比 `(create_time, user_id)` 更适合 `WHERE user_id=? AND create_time>?`，因为等值列用完后范围列才能继续用索引
- **区分度不是铁律**: 如果 a、b 总是同时做等值过滤，`(a,b)` 与 `(b,a)` 最终命中行数相同；再用前缀复用、排序需求和索引宽度取舍
...
**一句话**: 查询从该表需要的所有列都能从同一索引取得，无需回主键表，Extra 显示 `Using index`。

**90 秒**:
- **回表代价**: 二级索引叶子存的是主键值，回表要再做一次主键 B+ 树查询；页未命中 Buffer Pool 时，大量分散回表才进一步形成随机磁盘 IO
...
**90 秒**:
- **`type=index`**: 遍历整棵索引 B+ 树（全索引扫描）；比 ALL 略好，但仍是全扫。**注意 `type=index` 本身不代表不回表**——是否回表看 `Extra` 是否 `Using index`（覆盖索引才不回表）；若 `Extra` 是 `Using where` 之类，照样按主键回表取行
