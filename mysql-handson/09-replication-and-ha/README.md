...
**半同步**：主作者写完后，必须等**至少一个抄写员回信说「我收到了」**，才算这笔稿子发完。等了 10 秒（默认超时）没回信，退化成异步。好处：主库宕机时那份收到回信的从库一定有最新数据；坏处：每次写入多一个网络 RTT（Round-Trip Time，请求发出到收到响应的往返时间）。
...
**从库 IO thread**：接收 dump thread 推来的 binlog event，写到**本地 relay log**（格式和 binlog 几乎相同）。写成功后向主库回 ACK（Acknowledgement，确认回执；半同步的 ACK 也在这一步）。IO thread 几乎不会成为瓶颈，它只是网络搬运工。
...
# 或更优雅：在应用层停止写流量（切流量到新主 VIP（Virtual IP，虚拟 IP）之前）
