...
- 指定成功 run ID：`20260726T095829Z`。本机生成且被 Git ignore 的 archive 为 `mysql-handson/00-lab/ha/evidence/runs/primary-crash/20260726T095829Z/`；追溯本节至少读取 `verification.json`、`scenario-verification.json`、`events.jsonl`，session／五段 RTO 另来自 `session.json`、`timeline.jsonl`，写入分布来自 `written-by.txt`。archive 不是本次 commit 的内容，run ID 才是本地证据索引。
