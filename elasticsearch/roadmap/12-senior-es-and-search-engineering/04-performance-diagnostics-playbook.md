...
| `_cat/thread_pool` | `active`、`queue`、`rejected`、`completed`，重点看 `search`、`search_coordination`、`write`、`get`、`management`；ES 9.1+ bulk 协调再看 `write_coordination` | search/write 是否过载，是否存在 rejected requests 和队列堆积 |
...
- `_cat/thread_pool/search,search_coordination,write?v&h=node_name,name,active,queue,rejected,completed`
...
- 大量 scroll/export 或 `search_after` + PIT：scroll 和 PIT 生命周期内会保留搜索上下文或段引用；`search_after` 本身是无状态的游标式重复搜索，客户端慢消费仍会放大资源占用。
...
客户端收到 429 或 rejected execution，查询或写入失败率上升，重试后延迟更差。`search`、`search_coordination` 或 `write` thread pool 的 rejected 持续增长；ES 9.1+ bulk 协调还要看 `write_coordination`。

### first evidence

- `_cat/thread_pool/search,search_coordination,write,get?v&h=node_name,name,active,queue,rejected,completed`
- `_nodes/stats?filter_path=nodes.*.name,nodes.*.thread_pool.search,nodes.*.thread_pool.search_coordination,nodes.*.thread_pool.write,nodes.*.indices.search,nodes.*.indices.indexing,nodes.*.jvm,nodes.*.fs`
- ES 9.1+ 对 bulk 协调再看 `_cat/thread_pool/write_coordination?v&h=node_name,name,active,queue,rejected,completed` 和 `nodes.*.thread_pool.write_coordination`；旧版本没有该池时不要据此判断。
...
- search/search_coordination thread pool queue 满：高并发查询、慢查询、聚合、高亮、深分页、跨大量 shard fan-out。
- write thread pool queue 满：bulk 并发过高、小 bulk 太多、单文档写入风暴、pipeline 重、refresh/merge 跟不上；ES 9.1+ 可能先表现为 `write_coordination` queue/rejected。
...
- `_tasks?detailed=true&actions=*reindex,*byquery,*snapshot*,*search,*cluster*` 看长任务；对 by-query 或 snapshot 可先用 `actions=*byquery`、`actions=*/delete/byquery`、`actions=*snapshot*` 缩小范围，并从实际 `_tasks?detailed=true` 输出核对 action 名。
...
- 长期用 keyword/numeric/date + Doc Values、预聚合、downsampling/transform/OLAP、异步报表和 `search.max_buckets` guardrail；rollup 只作为存量集群的 legacy 方案看待。
...
- `_cat/thread_pool` 中协调入口节点 `search_coordination` 或 search queue/rejected 高。
...
- `_nodes/stats` 的 indexing time、indexing pressure、write queue/rejected 上升；ES 9.1+ bulk 协调场景还看 `write_coordination` queue/rejected。
...
“我会先用 slow log 判断 query phase 还是 fetch phase 慢；用 profile API 细分 query 和 aggregation；用 `_nodes/stats` 看 search、indexing、segments、merge、fs、jvm、breaker；用 `_cat/thread_pool` 看 search/search_coordination/write queue 和 rejected，ES 9.1+ bulk 协调再看 write_coordination；用 tasks API 找长任务；用 hot threads 判断 CPU 栈。这样才能把 query/fetch/aggregation/coordination/indexing/storage 分开，而不是把所有问题都叫查询慢。”
...
3. 看系统证据：`_nodes/stats` 看 search、indexing、segments、merge、fs、jvm、breaker；`_cat/thread_pool` 看 search/search_coordination/write queue 和 rejected，ES 9.1+ bulk 协调再看 write_coordination；tasks API 看 reindex、update_by_query、snapshot、long-running search；hot threads 看 CPU 栈；GC/heap 看 JVM memory pressure；circuit breakers 看是否有 request、fielddata、parent breaker。
