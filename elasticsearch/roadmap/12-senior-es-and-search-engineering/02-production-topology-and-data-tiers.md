...
- coordinating node 作为业务查询入口，承接客户端连接、查询 fan-out、TopN reduce、聚合结果合并和入口限流。
...
- coordinating 层作为统一查询入口，做认证、租户识别、查询改写、超时、分页限制、聚合限制、返回字段限制和慢查询采样。
...
- coordinating node 承接入口限流、超时、最大结果窗口、最大聚合 bucket 和慢查询采样。
...
- coordinating-only 入口隔离 fan-out、聚合 reduce、连接数和查询 guardrail。
...
5. 查询入口如果 fan-out 高、聚合重、多租户集中或连接数多，会用 coordinating node 隔离查询 reduce、入口限流、超时、分页和聚合 guardrail。
