...
- **写入可见性不是持久性保证**：refresh 后文档能被搜索到，只说明进入了可搜索 segment；业务写入是否真正可靠，还要看主库事务、消息投递、ES primary 写入、replica 复制、translog flush、节点故障和重试幂等。搜索“可见”不能等同于业务“已提交”。
...
- **low watermark**：节点磁盘超过低水位后，ES 通常不再把新的 shard 分配到该节点。风险是扩容和恢复时没有足够落点，replica 或新索引分配变慢。
- **high watermark**：节点磁盘超过高水位后，ES 会尝试把 shard 从该节点迁走。风险是 relocation 消耗 IO 和网络，如果全体节点都接近高水位，集群没有地方可迁。
- **flood-stage watermark**：节点达到 flood-stage 后，ES 可能把相关索引设置为 `index.blocks.read_only_allow_delete`，防止继续写入撑爆磁盘。即使后续腾出空间，也要确认水位稳定后再移除 read-only block。
...
5. 恢复写入：空间稳定后再清除 `index.blocks.read_only_allow_delete`，并验证写入、refresh、replica allocation 和查询延迟。
...
3. 如果是节点短暂丢失，先判断节点能否安全恢复；如果是磁盘水位，先释放空间或扩容，再恢复 read-only block；如果是 allocation 规则，修正 index settings、ILM 或节点角色。
