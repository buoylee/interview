...
如果需要修改字段类型，只能：
1. **标准方案（Reindex）**：创建新索引 `v2` + `_reindex` 迁移數據 + Alias 原子切换（零停机）。
2. **免 Reindex 替代方案**：
   - **新增子字段/新字段**：`PUT _mapping` 添加 `fields` (如多字段 `text` 增 `keyword`) 或 `new_field`（应用层切换读写）。
   - **Runtime Fields（ES 7.11+）**：动态计算字段，无需重建索引（适合低频查询救火）。

这个代价非常大，所以**在写入第一条数据之前就设计好 Mapping** 是最佳实践。

> **面试怎么答**："ES 的 mapping 能修改吗？"
>
> ES 的 Mapping 创建后只能追加新字段，不能修改已有字段的类型。这是因为字段类型决定了底层的索引结构（倒排索引、BKD 树、Doc Values 等），修改类型意味着要重建所有已有数据的索引，ES 不支持原地重建。如果确实需要修改，只能创建新索引配置正确的 Mapping，然后用 `_reindex` API 迁移数据，最后用 Alias 切换。若不想 Reindex，可考虑新增子字段 (multi-fields) 或使用 Runtime Fields 临时替代。
...
> **核心一句话**：
> - `text` = **分词** ➔ 构建 **倒排索引**
> - `keyword` = **不分词** ➔ 构建 **倒排索引** + **正排索引 (Doc Values)**

| 维度 | text | keyword |
|------|------|---------|
| **是否分词** | ✅ 会经过 Analyzer 分词 | ❌ 不分词，整体作为一个 Term |
| **索引结构** | 倒排索引（分词后的多个 Term） | 倒排索引（完整 Term） + Doc Values (正排) |
...
当你不预先定义 Mapping、直接写入文档时，ES 会根据 JSON 值自动推断字段类型。**注意**：Dynamic Mapping 按**字段首次出现**触发（而非仅第一条文档），任何文档出现未定义新字段均会自动追加 Mapping，但已有字段类型一旦确定即被冻结。
