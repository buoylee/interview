...
### 战场在哪：私有的 L1/L2，不是共享的 L3

先分清两件常被混为一谈的事：

- **缓存行（64 字节）不是某一层独有的东西**。L1、L2、L3 全都以 64 字节为一行来存数据——你读一个 4 字节的 `int`，CPU 也会把它周围凑满整整一行拉进来。所以「缓存行」本身不是问题所在。
- **问题出在「私有」上**。回看[第 1 节的层级图](#1-多核-cpu-架构与缓存层级)：L1、L2 是**每个核心各有一份**（Intel Nehalem 起、AMD Zen 都是私有 L2），L3 才是所有核心**共享一份**。

| 层级 | 归属 | 是否卷入 False Sharing 乒乓 |
|------|------|------|
| **L1** (I/D) | 每核私有 | ✅ 主战场 |
| **L2** | 每核私有 | ✅ 主战场 |
| **L3** (LLC) | 所有核共享 | ❌ 不乒乓，是失效后重新取数的「中转站」 |

正因为 L1/L2 是各核一份，硬件必须保证「同一块内存在不同核心的私有缓存里看到的值一致」——这就是**缓存一致性协议（MESI）**在背后干的活。False Sharing 的痛，痛在这个协议身上：

```
变量 A、变量 B 互不相关，却恰好落在同一条 64 字节缓存行里

Core 0 写 A → 这条 line 在 Core 0 的 L1 标记为 Modified
            → 协议广播：其他核心持有的副本全部 Invalidate（失效）
Core 1 写 B → 发现自己 L1 里这条 line 已失效
            → 从 Core 0 的缓存 / L3 重新拉取
            → 再标记 Modified，反过来又让 Core 0 失效
... 无限循环
```

这条 line 就在两个核心的**私有缓存（L1/L2）之间反复横跳**，业内叫 **cache line ping-pong（缓存行乒乓）**。两个核明明各写各的变量、逻辑上零竞争，却被「同住一条 line」拖成了串行。

**L3 为什么不乒乓**：它只有一份、大家共享，没有「多副本要同步」的问题。它的角色是——当私有缓存里那条 line 被失效后，核心去 L3（或对方的缓存）把最新值重新捞回来的来源。所以解 False Sharing 从来不是去动 L3，而是**让被不同核心高频写的变量别住进同一条 line**（下面的 padding / 对齐）。
