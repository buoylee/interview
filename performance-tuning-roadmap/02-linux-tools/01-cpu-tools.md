...
- **%usr**：用户态 CPU 使用率
- **%system**：内核态 CPU 使用率
- **%guest**：该进程花在**运行虚拟机 guest 代码**上的 CPU 比例。只有它本身是 KVM/QEMU 这类 hypervisor 进程(替虚拟机跑 vCPU)时才非 0；普通进程恒为 `0.00`——所以上表四个进程全是 `0.00`。（对应 `top` 的 `st` 是从**被偷 CPU 的客户机**那侧看,`%guest` 是从**宿主机替客户机干活**这侧看)
