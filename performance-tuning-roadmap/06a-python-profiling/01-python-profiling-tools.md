...
- **Y 轴**：调用栈深度
- **方块越宽**：表示该函数出现在越多采样调用栈中，通常代表它或其子函数占用较多被观察的执行时间；不等于该函数执行期间的系统 CPU 使用率
...
这里的百分比是**采样占比**，不是操作系统工具（如 `top`）显示的 CPU 使用率。例如共收集 1000 个有效样本，其中 352 个样本的调用栈顶端是 `process_data`，则其 `%Own` 约为 35.2%。

- **`%Own`**：函数自身位于调用栈顶端的样本比例
- **`%Total`**：函数自身或其子函数出现在调用栈中的累计样本比例

因此，`process_data` 的 `%Own = 35.20%` 表示它约占该进程被观察到的 CPU 执行样本 35.2%，不表示执行它时整台机器的 CPU 使用率是 35.2%。要知道进程实际使用了多少 CPU 核心容量，仍需查看 `top`、Prometheus 等系统指标。
