...
> 写密集业务（日志、时序、消息）选 LSM：所有写变顺序追加，后台 compaction 归并，写放大换读放大；读多写少（事务 OLTP）选 B+树：原地更新、范围读高效，随机写是代价但可接受。WAL（Write-Ahead Log）是通用手法 —— 把随机写转成一条顺序追加 + 后台异步落盘，崩溃靠重放，MySQL InnoDB redo log / Kafka log segment 都是这个原语的实例。设计文档里要写清楚：业务访问模式→引擎类型→放大系数→磁盘吞吐预算，一条链。
