...
> 🔬 **內幕**:`try_files` 不做 HTTP 重定向、不回客戶端任何回應。前面幾個參數(`$uri`、`$uri/`)是在**當前 location** 內檢查檔案/目錄是否存在,命中就直接服務、**不重走** location 匹配;但**最後一個回退參數若是一個 URI/路徑(如 `/index.html`),會觸發一次內部跳轉、重走 location 匹配**(這正是為什麼兜底寫成 `/` 會打回 `/` 這個 location → 死循環,見第 6 節)。只有回退到 `@命名 location` 或 `=code` 才不重走匹配。
